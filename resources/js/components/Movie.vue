<template>
  <div id="singleMovie">
    <div class="demo-card-wide mdl-card mdl-shadow--2dp" v-bind:style="{ 'background-image': 'url(' + `https://image.tmdb.org/t/p/original${singleMovie.backdrop_path}` + ')' }">
      ...
      <div class="section bg-dark">
        <div class="mdl-card__supporting-text text-light m-auto ">
          <span class="text-bold"><h4>Overview:</h4></span>  {{singleMovie.overview}}
        </div>
        <div class="mdl-card__supporting-text text-light m-auto ">
          <span class="text-bold"><h4>Rating:</h4></span>  <h5>{{singleMovie.vote_average}}/10 ({{singleMovie.vote_count}})</h5>
        </div>
      </div>
  </div>
</template>
